...
async def handle_series_request(update: Update, context: ContextTypes.DEFAULT_TYPE, series_id: int) -> None:
    ...
    if not db.increment_daily_usage(user_id):
        ...
        await update.message.reply_text(
            "<blockquote expandable>❌ Has alcanzado tu límite de búsquedas diarias.\n\n"
    ...
    if not series:
        await update.message.reply_text(
            "❌ Serie no encontrada.<blockquote expandable>Es posible que haya sido eliminada o que el enlace sea incorrecto.</tgconv></blockquote>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    try:
        ...
        await update.message.reply_text(
            f"📺 <b>{series['title']}</b>\n\n"
            f"<blockquote expandable>Selecciona un capítulo para ver o solicita todos los capítulos:</tgconv></blockquote>",
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML
        )
    ...
    try:
        ...
        try:
            ...
            message = (
                ...
                f"📝 <b>Sinopsis:</b>\n<blockquote expandable>{plot}</tgconv></blockquote>\n\n"
...
async def search_content(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text(
            "<blockquote expandable>Por favor, proporciona el nombre de la película o serie que deseas buscar.</tgconv></blockquote>\n"
    ...
    if not db.increment_daily_usage(user_id):
        ...
        await update.message.reply_text(
            "❌ Has alcanzado tu límite de búsquedas diarias.\n\n"
            "<blockquote expandable>Para continuar buscando, adquiere un plan premium:</tgconv></blockquote>",
            reply_markup=reply_markup,
            parse_mode=ParseMode.HTML
        )
        return
...
async def send_search_results(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str, results: list, status_message=None):
    ...
    if results:
        ...
        await status_message.edit_text(
            ...
            f"<blockquote expandable>Comprueba que escribes el nombre correctamente o utiliza variaciones del mismo. "
...
async def handle_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    profile_text = (
        ...
        f"<blockquote expandable>Nombre: {query.from_user.first_name}\n"
...
async def handle_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    info_text = (
        ...
        "<blockquote expandable>Si la película o serie no se encuentra en el canal, el bot te permitirá hacer un pedido.\n\n"
        ...
        try:
            ...
            if imdb_info:
                ...
                context.user_data['upser_description'] = (
                    ...
                    f"📝 <b>Sinopsis:</b>\n<blockquote expandable>{imdb_info['plot']}</tgconv></blockquote>\n\n"
